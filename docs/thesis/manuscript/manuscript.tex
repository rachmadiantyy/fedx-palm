...
%
% Running header/footer below reproduces the actual JUTIF-Template.docx header1.xml
% (journal name, Vol/No/Month/Year/Page, P-ISSN/E-ISSN, journal URL, DOI) and its
% page-number footer field -- both were only in the .docx header/footer parts, not
% the body text, so they weren't in the first LaTeX draft; added once the .docx's
% header1.xml was inspected directly.
...
\title{\vspace{-1.5em}\large Non-IID Federated Learning versus Centralized Training for Oil Palm Fresh Fruit Bunch Ripeness Detection Using YOLO11n}
\author{
  \todo{confirm co-author name(s) and order, e.g. supervisor/co-supervisor}$^{*1}$, Rachma Dianty$^{2}$ \\
...
\begin{figure}[htbp]
  \centering
  % TODO: insert the workflow diagram (dataset prep -> B1 & B2 training/eval paths); not a model output, draw separately (draw.io/PowerPoint) and place e.g. as figures/fig1_workflow.png
  \fbox{\parbox{0.85\linewidth}{\centering\vspace{2cm}\todo{insert workflow diagram: dataset prep $\to$ B1 (centralized) and B2 (federated) training/eval paths}\vspace{2cm}}}
...
\begin{figure}[htbp]
  \centering
  % Client distribution -- dataset partition stats, not a model output; can be table-only (Table 2) or a simple bar of Table 2's counts
  \fbox{\parbox{0.85\linewidth}{\centering\vspace{1.5cm}\todo{optional: bar chart of Table 2's per-client image counts (dataset partition stats, not a model result) -- may be skipped in favor of Table 2 alone}\vspace{1.5cm}}}
