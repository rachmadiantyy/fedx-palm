...
A matched centralized-versus-federated comparison was not conducted at this stage, because B2's held-out-test evaluation had not yet been completed at the time of writing; consequently, no centralized--federated utility difference is reported. B1's held-out test mAP50 (0.8820) and B2's three-seed mean validation mAP50 (0.8775, SD = 0.0157), reported in the preceding subsections, are on different splits and are not compared numerically here. \todo{RESULT NOT YET AVAILABLE: held-out test mAP50/mAP50-95 for each of B2's three locked seed checkpoints, needed to complete Table~\ref{tab:matched} below and establish a genuinely matched comparison.}
...
A matched centralized-versus-federated comparison is not conducted at this stage, because B2's held-out-test evaluation has not yet been completed; consequently, no centralized--federated utility difference is reported. B1's held-out test mAP50 (0.8820) and B2's validation-split mean (0.8775) are reported in \emph{Results} as separate, non-matched numbers on different splits, not as a comparison.
