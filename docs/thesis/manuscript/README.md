...
*(Catatan: nomor Gambar/Tabel di bawah sudah nomor final versi terbaru -- sempat geser semua sejak Tabel 3 & Gambar 4 baru soal client-class distribution ditambahkan.)*
...
- **Gambar 5** (confusion matrix/PR curve B1 di *held-out test* -- jalankan `scripts/42_generate_b1_test_plots.py`), **Gambar 6** (contoh deteksi kualitatif) -- masih ditunggu dari penulis.
- Gambar 3 (distribusi klien, jumlah gambar) sifatnya opsional -- angkanya sudah ada di Tabel 2.
- ~~Pernyataan ketersediaan kode/data, dan acknowledgement~~ (sudah diisi).
- ~~Tabel 6 (best-round vs final-round mAP50 seed 42 & 123)~~, ~~Tabel 7 (per-class AP50 tiga seed B2)~~ -- **sudah terisi dari `b2_k4_seed{42,123,2026}_history.json` yang sudah ada di repo ini**, tidak perlu training/evaluasi ulang. Temuan baru: kelas Underripe punya varians antar-seed jauh lebih besar (SD=0.0727) daripada mAP50 global (SD=0.0157) -- ini langsung menjawab supporting RQ ketiga di paper.
- **Masih tersisa**: Tabel 8 (matched held-out-test B1 vs B2 tiga seed) -- ini **butuh checkpoint B2 asli** yang cuma ada di server training kamu (`runs/b2_federated/k4_seed{42,123,2026}_leakagefree_stageA_seedfix/best_global.pt`), tidak bisa diisi dari sini. Command evaluasinya sudah dikasih di chat, tinggal jalanin dan kirim hasil print terminalnya.
- ~~Client-by-class breakdown~~ -- **sudah terisi** (**Tabel 3** baru + **Gambar 4** baru: stacked bar per-client class proportions), dari data yang penulis ekstrak sendiri dari partition manifest. Temuan menarik: quantity imbalance (Client 2 terbesar) TIDAK sejalan dengan class imbalance -- Client 2 justru paling seimbang komposisi kelasnya (JSD=0.0098), sedangkan Client 1 (terkecil) paling timpang (JSD=0.0422). Ini memperkuat Discussion soal FedAvg sample-weighted aggregation.
- ~~4 referensi `[NEEDS RECENT REFERENCE]` di Related Work~~ -- **sudah diisi** via web search (Kabala et al. 2023 Scientific Reports untuk agricultural FL, Internò et al. 2024 IJCNN untuk Non-IID convergence, Semmelrock et al. 2024 arXiv untuk reproducibility ML umum, Li et al. 2025 KDD/FedGuCci untuk multi-seed FL). **Catatan penting**: referensi ini didapat dari cuplikan hasil pencarian web, BUKAN dari membuka PDF aslinya langsung (akses ke arxiv.org/nature.com/ncbi.nlm.nih.gov diblokir kebijakan jaringan sandbox ini) -- sebelum submit, tolong cek ulang manual detail sitasinya (judul, penulis, DOI) di sumber aslinya buat mastiin akurat 100%.
