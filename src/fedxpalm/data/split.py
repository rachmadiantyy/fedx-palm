...
A prior version of this repo dropped this in favor of copying Roboflow's
own train/valid/test split as-is, to match related work's reported numbers.
That was reverted after an actual run on this exact dataset came back at
mAP@0.5=0.993 / mAP@0.5:0.95=0.906 -- numbers no genuine detector reaches
on a held-out set for 6 visually-similar ripeness classes, and a second,
independently-sourced palm-fruit dataset (tbs-hwiqz/kelapa-sawit) turned out
to share the exact same `frame<n>-<m>-...` filename convention, confirming
this is a structural property of how this kind of dataset gets captured
(sequential video/photo frames per bunch), not a one-off quirk -- so the
naive split isn't a safe default for this class of dataset in general.
...
against both this dataset's and tbs-hwiqz/kelapa-sawit's real filenames,
all of which follow `frame<anything>-<frame position digits>-...`.
"""
