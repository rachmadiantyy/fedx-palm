...
6 Classes (Roboflow alphabetical order):
  0: Abnormal, 1: Empty Bunch, 2: Overripe, 3: Ripe, 4: Underripe, 5: Unripe

Non-IID Distribution (Dir(α·1_C) per client):
  - Client A (α=0.1): Highly skewed (small α → sharp class concentration)
  - Client B (α=0.3): Moderately skewed
  - Client C (α=0.5): Mildly skewed
  - Client D (α=0.7): Near-uniform (large α → IID-like)
...
# 6 classes — order follows Roboflow source dataset (alphabetical)
# Source: dydy-worker/palm-fruit-ripeness-detection-f6sac-ccb2z v2
CLASS_NAMES = {
    0: "Abnormal",
    1: "Empty Bunch",
    2: "Overripe",
...
# Note: scalar α with Dir(α·1_C) controls *degree* of skew but not *which*
# class gets the majority. The dominant class per client is random per seed.
DIRICHLET_ALPHA_PER_CLIENT = {
    "client_1": 0.1,  # Client A: highly skewed (1-2 classes dominate ~70-90%)
    "client_2": 0.3,  # Client B: moderately skewed
    "client_3": 0.5,  # Client C: mildly skewed
    "client_4": 0.7,  # Client D: near-uniform (IID-like)
}
...
def main():
    ...
    logger.info("  Client A (α=0.1): Highly skewed (1-2 classes dominate)")
    logger.info("  Client B (α=0.3): Moderately skewed")
    logger.info("  Client C (α=0.5): Mildly skewed")
    logger.info("  Client D (α=0.7): Near-uniform (IID-like)")
