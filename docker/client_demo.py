...
def register():
    while True:
        try:
            ...
            if r.ok:
                log(f"Registered with server {SERVER} (α={DIRICHLET_ALPHA}, ε={DP_EPSILON})")
                return
...
def fake_local_training():
    """Simulates local fine-tuning on Non-IID data + DP injection."""
    ...
    log(f"Computing Δw (clipped to C=1.0, ε={DP_EPSILON}, σ={DP_SIGMA})")
    time.sleep(1)
    return delta_norm
...
def main():
    log(f"Starting (server={SERVER}, α={DIRICHLET_ALPHA}, ε={DP_EPSILON}, σ={DP_SIGMA})")
    register()
